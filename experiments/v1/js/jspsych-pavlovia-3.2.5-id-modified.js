...
jsPsych.plugins['pavlovia'] = (function() {
	...
	plugin.info = {
		...
		parameters: {
			...
			sessionId: {
				...
				default: 'SESSION',
				description: 'The session Id: "SESSION" (default) or any string'
			},
			errorCallback: {
	...
	const _save = async function(trial, data, sync = false)
	{
		...
		console.log(trial.participantId);
		console.log(trial.sessionId);
		console.log(_config.experiment);
		const key = _config.experiment.name + '_' + trial.participantId + '_' + trial.sessionId + '.csv';
