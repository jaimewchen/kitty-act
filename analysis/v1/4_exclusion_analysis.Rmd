---
title: "Kitty Act V1 Exclusion Analysis"
author: "Jaime Chen"
date: "2025-01-19"
output:
...
sampling_data_path_v1 <- here("processed_data", "kitty-act-v1-processed_data_sampling_v1.csv")
sampling_data_path_v1_1 <- here("processed_data", "kitty-act-v1-processed_data_sampling_v1_1.csv")

test_data_path_v1 <- here("processed_data", "kitty-act-v1-processed_data_v1.csv")
test_data_path_v1_1 <- here("processed_data", "kitty-act-v1-processed_data_v1_1.csv")

test_data_long_path_v1 <- here("processed_data", "kitty-act-v1-processed_data_long_v1.csv")
test_data_long_path_v1_1 <- here("processed_data", "kitty-act-v1-processed_data_long_v1_1.csv")
...
d_v1 <- d %>% filter(subject_id %in% v1_participants)
d_v1_1 <- d %>% filter(! subject_id %in% v1_participants)
...
sampling_data_v1_1 <- read_csv(sampling_data_path_v1_1) %>%
  mutate(number_training_images_order = case_when(
    number_training_image_order_vector == "[1,3]" ~ "One-Three",
    number_training_image_order_vector == "[3,1]" ~ "Three-One",
  ))
...
test_array_clean_v1_1 <- read_csv(test_data_path_v1_1) %>%
  mutate(number_training_images_order = case_when(
    number_training_image_order_vector == "[1,3]" ~ "One-Three",
    number_training_image_order_vector == "[3,1]" ~ "Three-One",
  ))
...
test_array_options_clean_v1_1 <- read_csv(test_data_long_path_v1_1) %>%
  mutate(number_training_images_order = case_when(
    number_training_image_order_vector == "[1,3]" ~ "One-Three",
    number_training_image_order_vector == "[3,1]" ~ "Three-One",
  ))
...
average_age_v1 <- participant_info %>% filter(subject_id %in% v1_participants) %>% 
  summarize(
    N=n(),
...
average_age_v1_1 <- participant_info %>% filter(! subject_id %in% v1_participants) %>% 
  summarize(
    N=n(),
    mean_age=mean(age_years,na.rm=TRUE),
    sd_age=sd(age_years,na.rm=TRUE),
...
participant_info_ages_v1 <- participant_info %>% 
  filter(subject_id %in% v1_participants) %>% 
  select(subject_id,age_group,age_dec,age_years,age_years_c)

participant_info_ages_v1_1 <- participant_info %>% 
  filter(! subject_id %in% v1_participants) %>% 
  select(subject_id,age_group,age_dec,age_years,age_years_c)

d_v1 <- merge(d_v1, participant_info_ages_v1)
sampling_data_v1 <- merge(sampling_data_v1, participant_info_ages_v1)
test_array_clean_v1 <- merge(test_array_clean_v1, participant_info_ages_v1)
test_array_options_clean_v1 <- merge(test_array_options_clean_v1, participant_info_ages_v1)

d_v1_1 <- merge(d_v1_1, participant_info_ages_v1_1)
sampling_data_v1_1 <- merge(sampling_data_v1_1, participant_info_ages_v1_1)
test_array_clean_v1_1 <- merge(test_array_clean_v1_1, participant_info_ages_v1_1)
test_array_options_clean_v1_1 <- merge(test_array_options_clean_v1_1, participant_info_ages_v1_1)
```
...
subj_test_accuracy_v1 <- test_array_clean_v1 %>%
...
sampling_data_long <-  sampling_data %>%
  group_by(participant,age_group,age_years,age_years_c,number_training_images,trial_counter, current_category_training_level) %>%
...
subj_test_prop <- test_array_options_clean %>%
  ...
  ungroup() %>%
  group_by(participant,age_years,age_group,age_years_c,number_training_images,current_category_training_level,training_condition,current_category_label_level,current_category_kind) %>%
  summarize(
