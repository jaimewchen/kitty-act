...
qualtrics_path <- here("..","data","processed","kitty-act-v1a_February 13, 2025_14.02.csv")

#### read in data ####
d <- read_csv(data_path)
prolific <- read_csv(prolific_path)
top_participants <- read_csv(here("..","data","processed", "top_participants.csv"))
...
unique_pavlovia_participants <- d %>%
  distinct(prolific_id,session_id,study_id,code, subject)

top_participants_vec <- as.character(top_participants$subject)
#top prolific ids
top_prolific <- d %>%
  distinct(prolific_id,session_id,study_id,code, subject) %>%
  filter(subject %in% top_participants$subject) %>%
  select(prolific_id) %>%
  mutate(bonus = 0.2)
write_csv(top_prolific,here("..","data","processed","top_prolific_ids.csv"))
