...
```{r}
m <- glmer(sample_subordinate_choice~age_years_c+(1|participant)+(1|current_category_kind),data=filter(combined_sampling_data,number_training_images==3 & sample_group=="children"),family=binomial)
summary(m)
m <- glmer(sample_basic_choice~age_years_c+(1|participant)+(1|current_category_kind),data=filter(combined_sampling_data,number_training_images==3 & sample_group=="children"),family=binomial)
summary(m)
m <- glmer(sample_superordinate_choice~age_years_c+(1|participant)+(1|current_category_kind),data=filter(combined_sampling_data,number_training_images==3 & sample_group=="children"),family=binomial)
summary(m)
m <- glmer(sample_outside_choice~age_years_c+(1|participant)+(1|current_category_kind),data=filter(combined_sampling_data,number_training_images==3 & sample_group=="children"),family=binomial)
summary(m)
```
...
```{r}
m <- glmer(is_exploratory~ sample_group_c*training_condition+(1|participant),data=combined_sampling_data,family="binomial",glmerControl(optimizer="bobyqa"))
summary(m)
Anova(m,type="III")
```
...
#model
#more complex random effects structure does not converge
...
p2 <- ggplot(combined_sampling_data,aes(age_years,is_exploratory, color=training_condition))+
  ...
  theme(axis.title = element_text(face="bold", size=24))+
  guides(color=guide_legend(title="Training Condition"))
ggsave(here(figure_path,"kitty-act-sampling-condition-opaque.png"),width=9,height=6,dpi=600)
...
m_sub <- glmer(prop_subordinate~training_condition+(1+training_condition|participant)+(1|current_category_kind),data=filter(subj_test_prop,sample_group=="children"),family=binomial)
summary(m_sub)
Anova(m_sub,type="III")

## basic choices
# training condition slope omitted due to singular fit
...
## basic choices
m_bas <- lmer(prop_basic~training_condition+(1+training_condition|participant)+(1|current_category_kind),data=filter(subj_test_prop,sample_group=="adults"))
summary(m_bas)
Anova(m_bas,type="III")
...
### H6: Interaction with Sample Group
...
### Exploratory: Interaction with age among children

```{r}
## subordinate choices
m_sub <- glmer(prop_subordinate~training_condition*age_years_c+(1|participant)+(1|current_category_kind),data=filter(subj_test_prop, sample_group=="children"),family=binomial)
summary(m_sub)
Anova(m_sub,type="III")

## basic choices
m_bas <- glmer(prop_basic~training_condition*age_years_c+(1|participant)+(1|current_category_kind),data=filter(subj_test_prop, sample_group=="children"),family=binomial)
summary(m_bas)
Anova(m_bas,type="III")

## superordinate choices
m_sup <- lmer(prop_superordinate~training_condition*age_years_c+(1|participant)+(1|current_category_kind),data=filter(subj_test_prop, sample_group=="children"))
summary(m_sup)
Anova(m_sup,type="III")

#set reference to 3_Item Broad level to further investigate broad condition
subj_test_prop$training_condition_ref_broad <- relevel(as.factor(subj_test_prop$training_condition), ref = "3-Item Broad")
m_sup <- lmer(prop_superordinate~training_condition_ref_broad*age_years_c+(1|participant)+(1|current_category_kind),data=filter(subj_test_prop, sample_group=="children"))
summary(m_sup)
Anova(m_sup,type="III")
```
...
## H6B/exploratory: No interaction between children and adults
...
```{r}
m <- glmer(is_chosen~match_categories*sampling_feedback+(1|participant)+(1|current_category_kind),data=combined_test_array_options_clean, family=binomial)
summary(m)
Anova(m, type="III")
```

#### Analysis focused just on sampled level

A slightly simpler analysis that focuses just on the sampled level for exploratory choices and includes an interaction with age group

```{r}
#center predictors
combined_test_array_options_clean <- combined_test_array_options_clean %>%
  mutate(sampling_feedback_c = ifelse(sampling_feedback == "positive",0.5,-0.5),
         sample_group_c = ifelse(sample_group == "adults",0.5,-0.5))

#interaction
m <- glmer(is_chosen~sampling_feedback_c*sample_group_c+(1+sampling_feedback_c|participant)+(1|current_category_kind),data=filter(combined_test_array_options_clean,match_categories=="sampled level" & sampling_choice_type == "explore"), family=binomial)
summary(m)

# just children
#removed the feedback slope due to convergence issues but the effect holds whether it is included or not
m <- glmer(is_chosen~sampling_feedback_c+(1|participant)+(1|current_category_kind),data=filter(combined_test_array_options_clean,match_categories=="sampled level" & sampling_choice_type == "explore"&sample_group=="children"), family=binomial)
summary(m)

# just adults
m <- glmer(is_chosen~sampling_feedback_c+(1+sampling_feedback_c|participant)+(1|current_category_kind),data=filter(combined_test_array_options_clean,match_categories=="sampled level" & sampling_choice_type == "explore"&sample_group=="adults"), family=binomial)
summary(m)
```
