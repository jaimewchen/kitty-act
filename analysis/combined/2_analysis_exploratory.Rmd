...
exp_adult_samptestint_missed <- exp_adult_samptestint %>% filter(sampling_consistent == 1) %>% mutate(is_not_chosen = -(is_chosen - 1)) %>% summarize(prop_sampconsistent_missed = sum(is_not_chosen)/sum(sampling_consistent), explored_yn, samp_feedback_type)

exp_kid_samptestint_missed <- exp_kid_samptestint %>% filter(sampling_consistent == 1) %>% mutate(is_not_chosen = -(is_chosen - 1)) %>% summarize(prop_sampconsistent_missed = sum(is_not_chosen)/sum(sampling_consistent), explored_yn, samp_feedback_type)
