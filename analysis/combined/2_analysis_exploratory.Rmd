...
plot1 <- confirm_feedback_summarized %>% ggplot(aes(x = test_choice_type, y = proportion, fill = samp_group)) + geom_col() + facet_wrap(~samp_group) + labs(title = "Confirmatory Sampling", ) + theme(legend.position="none", plot.title = element_text(hjust = 0.5)) + scale_x_discrete(labels = conf_labs) + xlab("Category Level of Test Extensions") + ylab("Proportion of Test Extensions")
...
patch <- ((plot1 + theme(plot.margin = unit(c(0,10,0,0), "pt"))) + (plot2 & ylab(NULL)) & xlab(NULL)) + plot_layout(widths = c(3, 5))
...
abbreviation <- c(">S", "<S,>T", "=S", "\u2264T")
meaning <- c("broader than sampling level", "narrower than sampling level, broader than training level", "same level as sampling level", "narrower or equal to training level")

key <- data.frame(abbreviation, meaning)
legend <- tableGrob(key, cols = NULL, rows = NULL, theme = ttheme_default(base_size = 10, padding = unit(c(1.5, 1.5), "mm")))

patch2 / legend + plot_layout(heights = c(6, 1))


# add pattern legend for feedback
...
patch2 / key_legend + plot_layout(heights = c(6, 1))

# move feedback legend onto children plot? or adjust location of key legend
