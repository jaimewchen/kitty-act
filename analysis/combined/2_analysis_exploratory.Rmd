...
```{r}
kid_choice_type_rate_2 <- kid_choice_type_rate %>% select(-age_mos) %>% mutate(age_years = as.character(age_years)) %>% mutate(age_group = as.character(age_group))


samptestint_choice_type_rate <- rbind(adult_choice_type_rate, kid_choice_type_rate_2)

samptestint_choice_type_rate %>% ggplot(aes(x = age_group, y = fa_rate)) + geom_violin()
samptestint_choice_type_rate %>% ggplot(aes(x = age_group, y = miss_rate)) + geom_violin()
