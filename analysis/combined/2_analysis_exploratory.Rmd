...
confirm_feedback_summarized <- feedback_summarized %>% filter(samp_feedback_type == "confirming") %>% select(samp_group, samp_feedback_type, same_or_below_training, above_conf_sampling) %>% pivot_longer(cols = c(same_or_below_training, above_conf_sampling), names_to = "test_choice_type")

explore_feedback_summarized <- feedback_summarized %>% filter(samp_feedback_type == "positive" | samp_feedback_type == "negative") %>% select(samp_group, samp_feedback_type, same_or_below_training, below_sampling_above_training, same_exp_sampling, above_exp_sampling) %>% pivot_longer(cols = c(same_or_below_training, below_sampling_above_training, same_exp_sampling, above_exp_sampling), names_to = "test_choice_type")


plot1 <- confirm_feedback_summarized %>% ggplot(aes(x = test_choice_type, y = value, fill = samp_group)) + geom_col() + facet_wrap(~samp_group) + labs(title = "did not explore", ) + theme(legend.position="none")

plot2 <- explore_feedback_summarized %>% ggplot(aes(x = test_choice_type, y = value, fill = samp_group)) + labs(title = "explored") + geom_col() + facet_grid(samp_feedback_type~samp_group) + theme(legend.position="none")

plot1 + plot2


# fix x tick labels, make into proportions?
