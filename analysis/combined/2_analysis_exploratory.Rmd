...
# * load in all data appropriately here (if there are data objects from 1_analysis_combined that we want to use, need to store them and then read them in here, so that the code can run in a standalone way)

# * make sure all packages needed to run the code are loaded

# * where does the function "sort_by" come from? Can it be replaced by a tidyverse function? (arrange)
...
exploratory <- combined_sampling_data %>% 
  ...
  arrange(sample_group,participant) %>%
...
exploratory_interact <- cbind(exploratory, fa_rate, choices) %>% 
  mutate(sample_group = case_when(
    ...
    TRUE ~ sample_group)) %>% 
  mutate(is_exploratory_yn = case_when(
    is_exploratory_yn == "explored" ~ "Explored",
    is_exploratory_yn == "did not explore" ~ "Did not explore",
    TRUE ~ is_exploratory_yn)) %>% 
  group_by(sample_group, is_exploratory_yn)
...
```{r}
# training inconsistent choices

exploratory_interact_plot_adult <- exploratory_interact %>% filter(sample_group == "Adults") %>% ggplot(aes(x = is_exploratory_yn, y = training_inconsistent_prop)) + 
  geom_jitter(aes(
    color = is_exploratory_yn),
    alpha = 0.4
    ) +
  geom_violin(aes(fill = is_exploratory_yn), alpha = 0.8) + 
  scale_color_manual(
    values = c("Explored" = "#F8766D",
               "Did not explore" = "#ffa7a1"
               )) +
  scale_fill_manual(
    name = "Explore Status",
    values = c("Explored" = "#F8766D",
  ...
  geom_signif(
              comparisons = list(c("Explored", "Did not explore")), 
              map_signif_level=TRUE) +
  ylab("Proportion of Training-Inconsistent Test Choices") +
  xlab("Did the Participant Explore During Sampling?") +
  labs(title = "Adults") +
  theme(plot.title = element_text(hjust = 0.5))

exploratory_interact_plot_kid <- exploratory_interact %>% filter(sample_group == "Children") %>% ggplot(aes(x = is_exploratory_yn, y = training_inconsistent_prop)) + 
  geom_jitter(aes(
    color = is_exploratory_yn),
    alpha = 0.4
    ) +
  geom_violin(aes(fill = is_exploratory_yn), alpha = 0.8) + 
  scale_color_manual(
    values = c("Explored" = "#00BFC4",
               "Did not explore" = "#73DEDE"
               )) +
  scale_fill_manual(
    name = "Explore Status",
    values = c("Explored" = "#00BFC4",
  ...
  geom_signif(
              ...
              map_signif_level=TRUE) +
  ylab("Proportion of Training-Inconsistent Test Choices") +
  xlab("Did Participants Explore During Sampling?") +
  theme(axis.ticks.y = element_blank(), axis.text.y = element_blank()) +
  labs(title = "Children") +
  theme(plot.title = element_text(hjust = 0.5))

exploratory_interact_patch1 <- exploratory_interact_plot_adult + (exploratory_interact_plot_kid & ylab(NULL)) & xlab(NULL)

exploratory_interact_plot <- wrap_elements(panel = exploratory_interact_patch1) +
  labs(tag = "Did Participants Explore During Sampling?") +
  theme(
    plot.tag = element_text(size = rel(1), vjust = 3),
    plot.tag.position = "bottom",
  )

exploratory_interact_plot

# Stats test

adult_explored_traininginconsistent <- exploratory_interact %>% filter(sample_group == "Adults", is_exploratory_yn == "Explored") %>% ungroup() %>% select(training_inconsistent_prop)

adult_noexplore_traininginconsistent <- exploratory_interact %>% filter(sample_group == "Adults", is_exploratory_yn == "Did not explore") %>% ungroup() %>% select(training_inconsistent_prop)

kid_explored_traininginconsistent <- exploratory_interact %>% filter(sample_group == "Children", is_exploratory_yn == "Explored") %>% ungroup() %>% select(training_inconsistent_prop)

kid_noexplore_traininginconsistent <- exploratory_interact %>% filter(sample_group == "Children", is_exploratory_yn == "Did not explore") %>% ungroup() %>% select(training_inconsistent_prop)

t.test(x = adult_explored_traininginconsistent, 
       y = adult_noexplore_traininginconsistent, 
       alternative = "greater")

t.test(x = kid_explored_traininginconsistent, 
       y = kid_noexplore_traininginconsistent, 
       alternative = "greater")
...
plot1 <- confirm_feedback_summarized %>% 
  ggplot(aes(x = test_choice_type, y = proportion, fill = samp_group)) + 
  geom_col() + 
  facet_wrap(~samp_group) + 
  labs(title = "Confirmatory Sampling", ) + 
  theme(legend.position="none", plot.title = element_text(hjust = 0.5)) + 
  scale_x_discrete(labels = conf_labs) + 
  ylim(0, 0.8) + 
...
abbreviation <- c(">S", "=S", "<S,>T", ">T", "\u2264T")
meaning <- c("broader than sampling level", "same level as sampling level", "narrower than sampling level, broader than training level", "broader than training", "narrower or equal to training level")
